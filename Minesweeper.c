...
// Continues to randomly generate Coords and traverse linked list to prevent duplicates
...
int pushCoords(CoordsNode * coords, CoordsNode * newCoord) {
    ...
    while (walker) {
        if (walker->ROW == newCoord->ROW && walker->COL == newCoord->COL) {
            free(newCoord);
            return 0;
        } 
        if (!walker->next)
...
// If '0' is entered, return 1 to quit game. Otherwise set row and col (offset by -1 for correct index) and return 0
int promptCoords(VisibleBoard * v, int * row, int * col) {
...
// Frees memory and ends program if player enters '0'
int main() {
    ...
    int quit; // Set by prompt functions. If 1 is returned, quit game
    int lost; // Set by updateVisibleBoard. If 1 is returned, player lost, prompt restart
    int col, row; // Player selected cords to select tile  
    while(1) {
        ...
        if (lost || valsBoard->tilesRevealed == ALLTILESREVEALED) { // Executes if player won or lost is true
            ...
            printBoard(display);  // If player has not won or lost, print updated board and continue game loop
    }
